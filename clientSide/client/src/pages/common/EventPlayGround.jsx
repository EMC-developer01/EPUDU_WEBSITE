...
export default function EventGalaxyPanel() {
    ...
    return (
        <section className="w-full min-h-[calc(100vh-75px)] pt-[75px] flex flex-col lg:flex-row text-white">
            ...
            <div className="relative flex-1 flex flex-col items-center justify-center px-4 py-10 overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <motion.p
                        className="mt-4 text-sm sm:text-base md:text-xl lg:text-2xl tracking-[0.35em] opacity-90 text-center"
                    >
                        EVENT MANAGEMENT
                    </motion.p>
                    ...
                    <motion.button
                        ...
                        className="mt-8 px-8 py-3 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 text-black font-bold"
                    >
                        Explore Events
                    </motion.button>
                </div>
                ...
                <div className="flex-1 relative overflow-hidden px-4 pb-10">
                    ...
                    >
                        {[...events, ...events].map((e, i) => (
                            <motion.div
                                key={i}
                                whileHover={{ scale: 1.05, x: 8, backgroundColor: "rgba(0,255,255,0.08)" }}
                                className="mx-auto mb-5 w-full max-w-[420px] rounded-xl border border-white backdrop-blur-md p-4 cursor-pointer"
                            >
                                <div className="flex items-center gap-4">
                                    <img
                                        src={`${IMAGE_BASE}/${e.image}`}
                                        alt={e.imageName}
                                        className="w-14 h-14 sm:w-16 sm:h-16 rounded-full object-cover border border-white"
                                    />
                                    <div>
                                        <h3 className="text-lg sm:text-xl font-bold">{e.imageName}</h3>
                                        <p className="text-xs sm:text-sm opacity-70">{e.description || ""}</p>
                                    </div>
                                </div>
                            </motion.div>
                        ))}
